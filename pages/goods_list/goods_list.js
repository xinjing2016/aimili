...
Page({
  ...
  nation:function(event){
    var nationid=event.target.dataset.nationid;//获取当前的国家id
    var index = this.data.select.indexOf(this.data.nation[nationid]["name"]);//看看这个id在nation里面有没有
    if(index==-1){//如果没有就在nation那里push
    ...
    if( this.data.selected_nation){  //国家子类选择不为空
...
/*
    ...
    console.log(this.data.brand[event.target.dataset.brandid]["name"]); 
    console.log(index);
    console.log(this.data);
    console.log(this.data.selected_brand);
 */   
